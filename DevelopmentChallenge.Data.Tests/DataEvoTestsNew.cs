...
namespace DevelopmentChallenge.Data.Tests
{
  ...
  public class DataEvoTestsNew
  {
    ...
    {
      Assert.That(_report.Generate(Array.Empty<IShape>(), "es"), Is.EqualTo("<h1>Lista vacía de formas!</h1>"));
    }

    [TestCase]
    public void TestResumeEmptyList_en()
    {
      Assert.That(_report.Generate(Array.Empty<IShape>(), "en"), Is.EqualTo("<h1>Empty list of shapes!</h1>"));
    }

    [TestCase]
    public void TestResumeEmptyList_it()
    {
      Assert.That(_report.Generate(Array.Empty<IShape>(), "it"), Is.EqualTo("<h1>Elenco vuoto di forme!</h1>"));
    }

    [TestCase]
    public void TestResumeEmptyList_langnotfound()
    {
      Assert.That(_report.Generate(Array.Empty<IShape>(), "ru"), Is.EqualTo("<h1>[empty-list]</h1>"));
    }

    [TestCase]
    public void TestResumenMultiShapes_es()
    {
      var formas = new IShape[]
      {
        new SquareShape(5),
        new CircleShape(3),
      ...
      var resumen = _report.Generate(formas, "es");
    ...
    public void TestResumenMultiShapes_en()
    {
      ...
      var resumen = _report.Generate(formas, "en");
    ...
    public void TestResumenMultiShapes_it()
    {
      ...
      var resumen = _report.Generate(formas, "it");
